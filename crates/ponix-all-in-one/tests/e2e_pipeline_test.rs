...
use ponix_nats::{
    ...
    ProcessedEnvelopeProducer, RawEnvelopeProducer,
};
...
async fn run_migrations(postgres_url: &str, clickhouse_native_url: &str) -> Result<()> {
    ...
    let pg_migrations_dir = workspace_root.join("crates/ponix-postgres/migrations");
    ...
    let ch_migrations_dir = workspace_root.join("crates/ponix-clickhouse/migrations");
    let ch_dsn = format!(
...
async fn run_consumers(
    ...
    let processed_processor = create_domain_processor(processed_envelope_service);
