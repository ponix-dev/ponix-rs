...
/// Parsed MQTT topic containing organization and end device IDs
...
pub fn parse_topic(topic: &str) -> DomainResult<ParsedTopic> {
    ...
    if parts.len() != 2 {
        return Err(DomainError::InvalidGatewayConfig(format!(
            "Invalid topic format '{}': expected '{{org_id}}/{{end_device_id}}'",
            topic
        )));
    }
    ...
    if end_device_id.is_empty() {
        return Err(DomainError::InvalidGatewayConfig(
            "End device ID cannot be empty in topic".to_string(),
        ));
    }
