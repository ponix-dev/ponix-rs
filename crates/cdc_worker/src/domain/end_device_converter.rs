...
pub struct EndDeviceConverter;

impl Default for EndDeviceConverter {
    fn default() -> Self {
        Self::new()
    }
}

impl EndDeviceConverter {
    pub fn new() -> Self {
        Self
    }

    fn value_to_proto(&self, data: &Value) -> anyhow::Result<EndDevice> {
        let end_device_id = data
            .get("end_device_id")
            .and_then(|v| v.as_str())
            .ok_or_else(|| anyhow::anyhow!("Missing end_device_id"))?
            .to_string();
...
#[async_trait]
impl CdcConverter for EndDeviceConverter {
...
mod tests {
    ...
    async fn test_convert_insert() {
        let converter = EndDeviceConverter::new();
        let data = json!({
            "end_device_id": "ed-123",
    ...
    async fn test_convert_update() {
        ...
        let bytes = result.unwrap();
        let decoded = EndDevice::decode(bytes).unwrap();
        assert_eq!(decoded.name, "Updated End Device");
    }
