use ponix_domain::{
    EmqxGatewayConfig, Gateway, GatewayConfig, GatewayOrchestrator, GatewayOrchestratorConfig,
    GatewayProcessStore, GatewayRepository, InMemoryGatewayProcessStore,
};
use std::sync::Arc;
...
mod mocks {
    ...
    use ponix_domain::error::DomainResult;
    ...
    impl MockGatewayRepository {
        ...
        pub fn get_gateway_count(&self) -> usize {
    ...
    impl GatewayRepository for MockGatewayRepository {
        async fn create_gateway(
            &self,
            _input: ponix_domain::CreateGatewayInputWithId,
        ...
        async fn update_gateway(
            &self,
            input: ponix_domain::UpdateGatewayInput,
        ) -> DomainResult<Gateway> {
            let mut gateways = self.gateways.lock().unwrap();
            if let Some(gateway) = gateways.get_mut(&input.gateway_id) {
                ...
                Err(ponix_domain::DomainError::GatewayNotFound(
                    input.gateway_id.clone(),
                ))
            }
        }
...
async fn test_orchestrator_starts_existing_gateways() {
    ...
    let config = GatewayOrchestratorConfig::default();
