...
use crate::grpc::{
    EndDeviceDefinitionServiceHandler, EndDeviceServiceHandler, GatewayServiceHandler,
...
pub struct PonixApiServices {
    pub end_device_service: Arc<EndDeviceService>,
...
impl PonixApiServices {
    ...
    pub fn into_routes(self, config: &GrpcServerConfig) -> Routes {
        // Create handlers
        let end_device_handler =
            EndDeviceServiceHandler::new(self.end_device_service, self.auth_token_provider.clone());
        ...
        builder
            .add_service(
                EndDeviceServiceServer::new(end_device_handler)
                    .max_decoding_message_size(max_decode)
                    .max_encoding_message_size(max_encode),
            )
            .add_service(
                EndDeviceDefinitionServiceServer::new(definition_handler)
                    .max_decoding_message_size(max_decode)
                    .max_encoding_message_size(max_encode),
            )
            .add_service(
